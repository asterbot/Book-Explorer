...
DROP MATERIALIZED VIEW IF EXISTS production.v_user_genre_score CASCADE;
CREATE MATERIALIZED VIEW production.v_user_genre_score AS
WITH base AS (
    -- rated
    SELECT ur.userid, bg.genreid, ur.rating AS raw_score
    FROM   production.userrating ur
    JOIN   production.bookgenre bg ON bg.bookid = ur.bookid
    ...
    JOIN   production.bookgenre bg ON bg.bookid = up.bookid
    WHERE  NOT EXISTS (
        SELECT 1
        FROM   production.userrating ur
...
CREATE UNIQUE INDEX v_user_genre_score_pk
  ON production.v_user_genre_score(userid, genreid);
...
DROP MATERIALIZED VIEW IF EXISTS production.v_club_genre_score CASCADE;
CREATE MATERIALIZED VIEW production.v_club_genre_score AS
SELECT bcr.clubid,
       bg.genreid,
       COUNT(*) * 10.0 AS score
FROM   production.bookclub_reads bcr
JOIN   production.bookgenre bg ON bg.bookid = bcr.bookid
GROUP BY bcr.clubid, bg.genreid;

CREATE UNIQUE INDEX v_club_genre_score_pk
  ON production.v_club_genre_score(clubid, genreid);

-- similarity calc function
DROP FUNCTION IF EXISTS production.suggest_club(INT);
-- argument is the user that we care about
-- this function suggests club based on cos similarity. 
CREATE FUNCTION production.suggest_club(p_user INT)
...
AS $$
    ...
    already_in AS (
        SELECT clubid
        FROM   production.bookclub_members
        WHERE  userid = p_user
    ),
    -- get the user's genre vector
    uv AS (
        SELECT genreid, score
        FROM   production.v_user_genre_score
        WHERE  userid = p_user
    ),
    ...
    dot_and_norms AS (
        ...
        FROM   production.v_club_genre_score cgs
...
DROP FUNCTION IF EXISTS production.refresh_reco_views();
CREATE FUNCTION production.refresh_reco_views()
RETURNS void LANGUAGE plpgsql AS $$
BEGIN
    REFRESH MATERIALIZED VIEW CONCURRENTLY production.v_user_genre_score;
    REFRESH MATERIALIZED VIEW CONCURRENTLY production.v_club_genre_score;
END $$;

-- refresh
DO $$ BEGIN
  PERFORM production.refresh_reco_views();
END $$;

SELECT clubid FROM production.suggest_club(1);
